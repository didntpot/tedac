...
func (pk *AvailableCommands) marshal(w *protocol.Writer) {
	...
	for _, command := range pk.Commands {
		legacyprotocol.WriteCommandData(w, &command, enumIndices, suffixIndices, dynamicEnumIndices)
	}
...
func (pk *AvailableCommands) enums() (enums []legacyprotocol.CommandEnum, indices map[string]int) {
	indices = make(map[string]int)

	for _, command := range pk.Commands {
		if len(command.Aliases) > 0 {
			aliasEnum := legacyprotocol.CommandEnum{Type: command.Name + "Aliases", Options: command.Aliases}
...
func (pk *AvailableCommands) dynamicEnums() (enums []legacyprotocol.CommandEnum, indices map[string]int) {
	indices = make(map[string]int)
